...
describe('spell library UI structure', () => {
  ...
  it('uses a full-width scrollable list with editing in a modal dialog', () => {
    ...
    expect(component).not.toContain("t('library.emptyRecommendationsLead')");
    expect(component).not.toContain("t('library.emptyRecommendationsTail')");
    expect(component).not.toContain('emptyRecommendationsDescription');
    expect(component).toContain('candidate-memory-reveal phase-${recommendationIntroPhase}');
    expect(component).toContain("type RecommendationIntroPhase = 'opening' | 'message' | 'ready'");
    expect(component).toContain('RECOMMENDATION_MESSAGE_DELAY_MS');
    expect(component).toContain('RECOMMENDATION_READY_DELAY_MS');
    expect(component).not.toContain('RECOMMENDATION_INTRO_SESSION_KEY');
    expect(component).not.toContain('window.sessionStorage');
    expect(component).toContain('recommendationIntroStartedRef');
    expect(component).toContain('recommendationIntroCompletedRef');
    expect(app).toContain("if (view === 'library')");
    expect(app).toContain('<LibraryView');
    expect(component).toContain("recommendationIntroPhase === 'message'");
    expect(component).toContain("recommendationIntroPhase === 'ready'");
    expect(
    ...
    expect(component).not.toContain('WandSparkles');
    expect(component).not.toContain('Footprints');
    expect(bookScene).toContain('new THREE.WebGLRenderer');
    expect(bookScene).toContain('new THREE.PerspectiveCamera');
    expect(bookScene).toContain('RoundedBoxGeometry');
    expect(bookScene).toContain('new THREE.CanvasTexture');
    expect(bookScene).toContain('createArcaneCircle');
    expect(bookScene).toContain('new THREE.LineLoop');
    expect(bookScene).toContain('new THREE.LineSegments');
    expect(bookScene).toContain('arcaneField.rotation.z');
    ...
    expect(component).not.toContain('candidate-memory-searchlight');
    expect(component).not.toContain('BrainCircuit');
    ...
    expect(styles.match(/\.candidate-empty-state\s*\{[^}]+width: 100%;[^}]+height: 100%;[^}]+\}/s)?.[0]).toBeTruthy();
    expect(styles).toContain('background: #121114');
    expect(styles).not.toContain('restricted-section.jpg');
    expect(styles).toContain('.candidate-memory-three-stage');
    expect(styles).toContain('.candidate-memory-three-canvas');
    expect(styles.match(/\.candidate-memory-reveal\s*\{[^}]+position: relative;[^}]+/s)?.[0]).toBeTruthy();
    expect(styles.match(/\.candidate-memory-phrase\s*\{[^}]+position: absolute;[^}]+top: 316px;[^}]+/s)?.[0]).toBeTruthy();
    expect(styles).toContain('@keyframes candidate-memory-phrase-character');
    expect(styles).toContain('calc(var(--character-index) * 42ms)');
    expect(styles.match(/\.candidate-empty-action\s*\{[^}]+top: 315px;[^}]+/s)?.[0]).toBeTruthy();
    expect(styles).toContain('@keyframes candidate-memory-action-arrive');
    expect(styles).toContain('@media (prefers-reduced-motion: reduce)');
    expect(styles).not.toContain('candidate-memory-diagram-pulse');
    expect(styles).not.toContain('phase-ready .candidate-memory-book');
